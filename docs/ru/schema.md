...
`width` — legacy (используется только для границ валидации); раскладку rich-сообщений делает Telegram, а не фиксированная ширина.

`id_prefix` (по умолчанию `OL`) — префикс для id задач, создаваемых из CLI (`openlinear issue add` без `--id`): префикс плюс следующий номер после максимального числового суффикса.
